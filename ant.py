...
def prop_successful_ants(matches: list, threshold: int = 80) -> float:
    # TODO: set default threshold value, just taking a random guess here based on the default run
    return np.sum(np.where(np.array(matches) < threshold)) / len(matches)

def avg_final_match(matches: list) -> float:
...
class Ant:
    ...
    def decide_next_position(self, network: LatticeNetwork, q: float = 0.2, r: int = 1, search: bool = False) -> bool:
        ...
        if rng.uniform() < q:
            ...
            # stop condition - current pheromone is better than best neighbor
...
if __name__ == "__main__":
    ...
    success_prop = prop_successful_ants(min_diffs, threshold=80)
    avg_match = avg_final_match(min_diffs)
    
    print(f"Percentage of successful ants: {100*success_prop}%")
    print(f"Average final match: {avg_match}")
